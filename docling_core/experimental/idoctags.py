...
_DEFAULT_CONTENT_TYPES: set[ContentType] = {ct for ct in ContentType}
        ...
        **kwargs: Any,
        ...
        if (
            (
                ...
                and ContentType.TEXT_CODE in params.content_types
            )
            or (
                isinstance(item, FormulaItem)
                and ContentType.TEXT_FORMULA in params.content_types
            )
            or (
                not isinstance(item, (CodeItem, FormulaItem))
                and ContentType.TEXT_OTHER in params.content_types
            )
        ):
        ...
        if item.self_ref not in doc_serializer.get_excluded_refs(**kwargs):
            ...
            if ((not is_chart) and ContentType.PICTURE in params.content_types) or (
                ...
                if chart_data and chart_data.table_cells:
                    ...
                    otsl_payload = _wrap(
                        text=otsl_content, wrap_tag=IDocTagsToken.OTSL.value
                    )
                    body += otsl_payload
